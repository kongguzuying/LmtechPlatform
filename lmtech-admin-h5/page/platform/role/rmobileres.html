...
<div class="row">
    <div class="col-lg-12">
        <section class="panel">
            <form action="${pageContext.request.contextPath}/rmobileres/check.do" method="post">
                <div class="adv-table">
                    <div class="row-fluid">
                        <div class="span3">
                            <div id="dynamic-table_length" class="dataTables_length">
                                ...
                                <div class="btn-group">
                                    ...
                                    <a href="${pageContext.request.contextPath}/rmobileres/checklist.do?id=${id}&type=0"
                                       class="btn <c:choose><c:when test="${type==0}">btn-success</c:when><c:otherwise>btn-default</c:otherwise></c:choose> tb-btn">所有资源</a>
                                    <a href="${pageContext.request.contextPath}/rmobileres/checklist.do?id=${id}&type=1"
                                       class="btn <c:choose><c:when test="${type==1}">btn-success</c:when><c:otherwise>btn-default</c:otherwise></c:choose> tb-btn">已授权资源</a>
                                    <a href="${pageContext.request.contextPath}/rmobileres/checklist.do?id=${id}&type=2"
                                       class="btn <c:choose><c:when test="${type==2}">btn-success</c:when><c:otherwise>btn-default</c:otherwise></c:choose> tb-btn">未授权资源</a>
                    ...
                    <h:pager pager="${pageData}"
                             url="${pageContext.request.contextPath}/rmobileres/checklist.do?type=${type}&id=${id}"></h:pager>
                </div>
            </form>
        </section>
    </div>
